// api/date-product.js
// Extracts a probable product/service/site launch date from one chat.
...
function extractDate(texts){
  ...
  for(const t of texts){
    ...
    // Only consider product/service/website mentions (avoid “campaign”)
    if (!/(product|service|site|website|platform|app|release|go live|launch)/i.test(low)) continue;
    if (/campaign/i.test(low)) continue;
    ...
    if (m4 && monthIdx(m4[2])) candidates.push({ y:null, m:monthIdx(m4[2]), d:null }); // “xx.MM”
  }

  // choose the most recent mention in the text order
